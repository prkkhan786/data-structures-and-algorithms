...
public class Graph2 {
	...
	public void hamiltoniun(String src) {
		HashMap<String, Boolean> vis = new HashMap<>();
		hamiltoniunhelper(src, vis, "", src);
	}

	private void hamiltoniunhelper(String src, HashMap<String, Boolean> vis, String psf, String osrc) {
		if (vis.size() == vecs.size() - 1) {
			System.out.print(psf + src + " is a path" + " ");
			if (containedge(src, osrc)) {
		...
		for (String n : vecs.get(src).keySet()) {
			if (!vis.containsKey(n)) {
				hamiltoniunhelper(n, vis, psf + src, osrc);
			}
		}
		vis.remove(src, false);
	}
